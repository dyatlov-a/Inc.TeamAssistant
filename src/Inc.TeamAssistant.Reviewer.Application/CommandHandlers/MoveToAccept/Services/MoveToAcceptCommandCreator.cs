...
internal sealed class MoveToAcceptCommandCreator : ICommandCreator
{
    ...
    public Task<IRequest<CommandResult>?> Create(MessageContext messageContext, CancellationToken token)
    {
        ...
        if (messageContext.Text.StartsWith(CommandList.Accept, StringComparison.InvariantCultureIgnoreCase))
        {
            var storyId = Guid.Parse(messageContext.Text.Replace(CommandList.Accept, string.Empty));
